...
## 2026-09-06（2）「環境判定が通る」を「規則本文が届く」と取り違え、必読を塞いだまま完了と報告した（同日の再発）

- やらかし：ガードに `preflight` op を足し、`CLAUDE.md` の必須手順を満たせるようになったと報告した。実際には固定readerが `checkedPath` でリポジトリ内に限定されており、**必読の `C:/AI/vault/WORKFLOW.md`・`rules/corrections.md`・`rules/mistakes.md`・`Memory.md`、`C:/AI/web-development/CLAUDE.md` はいずれも読めなかった。**しかもフックは `allow` を返し、実行時に `Edit path is outside repository.` で落ちるという、最も気付きにくい形だった。Codex に指摘されるまで気付かなかった。
- 原因：**同日の朝に自分で書いた再発防止「allow 側の経路（正常な読み取り）が実際に通ることを必ず1件確認する」を、`preflight` の1件だけで満たしたことにした。**`preflight` は「上位層ファイルが読める状態か」を報告するだけで、本文を届けない。判定器が ok を返すことと、規則が読み手に届くことは別の事象である。前者を後者の証拠として扱った。
- 再発防止：**必読・必須と定めた資源は、「参照できる状態か」ではなく「実際に中身が返るか」を検査する。**判定・診断・ヘルスチェックの類が ok を返しても、それは成果物が届いた証拠にならない。強制器に経路を足したら、その経路が**目的の文書を1件以上、バイト数付きで返す**ことを機械検査に入れる（`selftest` の `required reading arrives:`）。加えて、その検査自体を消す変異を当てて E2E が落ちることまで確かめる。1度目の変異試験では気付かず、検査が消えても E2E が通る状態だった。

## 2026-09-06 外部ツールの入力形を想像で決めて強制器を書き、Codexの全操作を停止させた

- やらかし：`tools/codex-edit-guard.mjs` の PreToolUse 判定を、Codex の実際のフック入力ではなく
  想像した形（`{command, cmd, workdir, login, tty, yield_time_ms, max_output_tokens}`）に対して書いた。
  `input.login === false` を必須にしたため、**正規の固定 reader コマンドまで拒否**され、
  信頼・有効化した瞬間から Codex は読み取りも自己修正もできなくなった。E2E 18群は全て PASS していたが、
  その E2E が同じ想像上の形を再生していたため、欠陥を一度も検出しなかった。
- 原因：**強制器と試験の両方を、同じ未確認の前提の上に建てた。**公式仕様は
  `learn.chatgpt.com/codex/hooks.md` に「`tool_name` は正規化名（`Bash` 等）、`Bash` と `apply_patch` は
  `tool_input.command` を使う」と明記されており、`login` は存在しない。加えて `~/.codex/logs_2.sqlite` の
  `codex_core::tools::router` 行に実際の拒否理由が全て残っていたが、どちらも実装前に見ていなかった。
- 再発防止：**外部エージェント・外部ツールの入出力契約を検査する強制器を書くときは、着手前に
  (1) 公式ドキュメントの当該フィールド定義と (2) 実機ログに残る実際のペイロードの2つを取得し、
  E2E の fixture をその実測値から作る。**想像した形だけを再生する E2E は、通っても根拠にしない。
  強制器を有効化したら、`allow` 側の経路（正常な読み取り）が実際に通ることを必ず1件確認する。
  `deny` 側だけを確認して「効いている」と判断しない。
