...
次のいずれか。**フックが有効な間、その下にいるエージェントは修理役になれない**（`protectedPath` がガード本体と `.codex/` の編集を拒否するため）。逆に言えば、修理役の条件は「別のAIであること」ではなく「**その時点でフックの下にいないこと**」である。

- **A：対象外のクライアント。**このフックは Codex の PreToolUse であり、他のクライアントには適用されない。P3 を経ずに直せるため、**ガードを止める時間帯が発生しない**。安全性はこちらが上で、既定とする。
- **B：P3 でフックを止めた後の、同じ Codex。**フックを止めればその Codex はもう統制下に無いので、自分で修理してよい。A のクライアントが無い環境ではこちらを使う。⚠️ 止めている間は統制が存在しないため、**修理以外の作業を一切しない**。修理が終わったら P5 を通してから必ず戻す。
