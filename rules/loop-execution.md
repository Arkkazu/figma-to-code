...
**既定のレビュー役は同一エージェントの別contextセッション**（共通Vault `rules/corrections.md` 2026-08-26）。「実装役だから書けない」は停止理由にしない。成立条件は「actor または contextId の一方が実装役と異なること」で、**actorの相違は必須にしない**（必須にすると全レビューが他エージェント待ちになる）。

actorが同じときは独立性の根拠が contextId の申告だけになり、自己承認を止められない（`rules/corrections.md` 2026-08-25）。そこで `reviewEvidencePath` に次を満たす受領証を置く。`figma-gate.mjs` が `new` 決定で機械検査する。

1. `sameActorDisclosure`：同一セッションが実装にも関与した事実（20文字以上）
2. `searchEvidenceSha256`：検索証跡のSHA-256。検証器が実体から計算し直す
3. `reviewedRationale` / `reviewedSearchQueries`：レビュー時点で読んだ決定本文の逐語。同上

独立性の証明ではない。得られるのは、承認を黙って書けないこと、承認後に決定・証跡を書き換えると失効すること、同一actorレビューが後から数えられることの3点である。レビュー側は実装役の申告値を使わず実体から計算し直し、変更を壊す負のテストで検査が効くことを確かめる。