...
- **解決（2026-09-04、オーナー指示「独立レビューが仕様だと作業効率が悪すぎる」）**：この失敗の前提だった**独立レビューの工程そのものを廃止した**ので、上の「actorの相違を必須にする」案は採らない。同日、まず「actorが同じときだけ受領証を要求する」検査を実装したが、オーナーから独立レビュー自体が不要と指示され、受領証ごと撤去した。`figma-gate.mjs` の `new` 決定から `independentApproved` / `reviewerActor` / `reviewerContextId` / `reviewedAt` / `reviewEvidencePath` を削除し、`figma-log-promote.mjs` からレビュー役と実装役の相違要求を削除した。**残した検査**：検索語が証跡に実在するか、`codePath` が `changeTargets` にあるか、根拠ハッシュ、負のE2E、弱体化の実測、オーナー承認。**自己承認の余地は「承認そのものを廃止した」ことで消えた**（承認フィールドが無いので偽装する対象が無い）。規則本文は `rules/loop-execution.md`「独立レビューを工程にしない」。
