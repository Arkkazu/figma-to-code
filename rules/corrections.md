...
- 今後：所有の割り当てをscopeに紐づけ、close時に解放する。台帳の各行へ、どのscopeの求めで割り当てたかを持たせ、そのscopeがclosedまたはabortedなら所有は失効した扱いにする。監査は、失効した所有と、実在するのに所有者が未登録のパスを一覧で出し、解除に必要な台帳差分をそのまま貼れる形で提示する。宣言時点で落ちるべき検査が、commit直前まで表面化しない状態を作らない。
...
- **実装済み（2026-09-04、オーナー指示「先に潰しておくべき地雷を対応しろ」）**：上の「actorの相違を必須にする」案は**採らない**。共通Vault `rules/corrections.md` 2026-08-26（既定のレビュー役は同一エージェントの別contextセッション。「実装役だから書けない」を停止理由にしない）と正面から衝突し、採ればすべての独立レビューが他エージェント待ちになるためである。優先順位は `rules/loop-execution.md`「優先順位」により上位層が勝つ。代わりに、actorが同じときだけ受領証を決定へ束ねる検査を `templates/verify/figma-gate.mjs` の `new` 決定に追加した。要求するのは (1) 同一セッション関与の開示20文字以上 (2) 検索証跡のSHA-256 (3) rationale と searchQueries の逐語で、(2)(3) は検証器がファイル実体から計算し直すため、承認後に決定や証跡を書き換えると approved が失効する。負のE2E6件（受領証なし・対象外・開示不足・ハッシュ不一致・rationale書き換え・searchQueries書き換え）と対称ケース2件（受領証が揃えば通る／actorが異なれば要求しない）を `figma-gate.e2e.mjs` に追加し、**呼び出しを無効化すると1件目で落ちることを実測した**。規則本文は `rules/loop-execution.md`「独立レビューの成立条件」。独立性の証明ではないことは規則本文に明記した。
