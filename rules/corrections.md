...
- 今後：Figmaのpreflight・checkpoint・section-close・closeはコーディング反復内だけで実行する。Git hook、commit、push、deployでは実行・再実行・完了条件化しない。未検証scopeは「Figma実装完了」とは報告しないが、Git操作を止める条件にはしない。
