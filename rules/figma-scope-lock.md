...
## 台帳の予約は、受領証を持つときだけ他担当を止める（2026-09-04 追加）

scope coordination 台帳の `active` / `waiting` 行は**予約**であって、進行中の編集ではない。
両gateとも対象ファイルを編集する前に preflight 受領証を要求するため、**live な受領証を
1件も持たない scope は定義上まだ1行も編集していない**。守るべき編集が存在しないのだから、
その行が他担当の宣言を止める理由は無い。

したがって `scope-conflict-audit` は、台帳行を停止事由にするのは**その scope が live な
gate受領証（phase が closed / aborted 以外）を保持しているときだけ**とする。受領証を
持たない行が交差した場合は、担当者・台帳ステータス・manifest の最終更新日数を添えて
NOTE で報告し、宣言は止めない。並行編集の排他は受領証claimの交差判定が担う。

止めるときは、交差判定の枝でも**担当者と受領証の phase と次の行動**を必ず出す。
2026-09-01 に所有台帳の枝へ入れた「貼れば直る差分まで出す」規則を、こちらの枝は
適用していなかった。`FAIL: <path> は <id>（coordination:active）と競合します。` の
...
止まっていた。しかもその行自身の `changeTargets` と note は「2026-09-02 に対象外とした」と
記録しており、**manifest だけが古い宣言を残していた**（監査は manifest を読む）。

これは 2026-09-01 の `stale-path-ownership-blocks-other-actor` と同じ形である。当時の修正で
所有台帳には失効を入れたが、同じ「解放されない保持」が台帳の予約行へ移っただけで残って
いた。**宣言時に落ちるべき検査が commit 直前まで表面化しない状態を作らない。**

弱体化の実測：同案件の live scope 10件を旧実装と新実装で走らせ、FAIL 総数 **4 → 2**。
消えた2件はいずれも受領証を持たない予約による停止で、受領証を保持する2件は従来どおり
止まる。

契約の回帰試験は `templates/verify/scope-conflict-audit.e2e.mjs`（11件）。空台帳の受理、
空台帳でも交差する並行scopeが止まること、配列でない台帳の拒否を追加した。失効の読み飛ばし、
稼働中の所有による停止、休眠所有の明示と解除差分、未登録を止めないこと、交差する並行scopeが
止まること、不正な `grantedForScope` の拒否を固定する。2026-09-04 に (j) 受領証を持たない
