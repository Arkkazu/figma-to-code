...
- level: L2（節の確定は批評役の合格＋人間承認）
- owner: kazu
- agents:
  - 起草役: 開始時にkazuが指定する。`spec/` の該当節を執筆し、QUESTIONS.md の状態を更新する
  - 批評役: 開始時にkazuが指定する。起草役との兼任を禁止する。同一エージェントを充てる場合は、起草に使ったcontextとは別のcontextであること（`spec/12-loop-design.md` §「actorまたはcontextの少なくとも一方が異なる」と同じ条件）。特定のエージェントを既定の批評役として固定しない
...
4. 批評役が独立に批評する。観点: 曖昧さ / 根拠・出典 / 合否基準の検証可能性 / 設問への完全な回答
5. 不合格 → 指摘を STATE.md に記録し、次イテレーションで修正。合格 → 状態を `合格（承認待ち）` にする
...
- ログ昇格: 同じ再発キーが閾値に達したときだけ、対象ルール・検証器・負のE2Eを含む `pending-review` 提案を出す。独立批評とkazuの承認まで正本には反映しない。
- 昇格: 提案は差し戻し中のQ-12に紐付け、独立批評とkazuの承認があるまで仕様・ルール・テンプレートの確定状態にしない。
- D-011起草: 横断ログは `record`、再発proposalは `scan`、独立レビューは `review`、承認済み限定差分は `apply` を使う。`apply` は通常scopeのclose後処理には含めず、別の仕様育成イテレーションでだけ実行する。
...
- 禁止: 学習結果を理由に、Figma照合、W3C、独立レビュー、人間ゲートを弱めない。Git hook、commit、push、deployにも結び付けない。
## ガードレール（Guardrails）
