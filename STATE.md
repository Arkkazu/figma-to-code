...
- 追補：実装commit `73ba1ed`、配布E2Eの実フック参照先整合修正 `8b70b7f`。全検査の初回は19 PASS / 1 TIMEOUT（600秒）。当該1本は単独再実行で22段859 assertions PASS（537秒）。配布先も859 assertions PASS（529秒）、例外連携83 assertions PASS（55秒）で正式配布成功。初回19/20を20/20へ読み替えず、全20本の合格証跡が揃ったこととして記録する。凍結ハッシュの拒否条件は不変で、旧案件受領証は書き換えていない。
...
- 完了（同日 / 有効化）: ownerの指示で残2件を実施した。`allow_auto_merge: true`、Actions の `can_approve_pull_request_reviews: true`。ruleset id `22345309` を `active` で作成し、`rules/branches/master` が deletion / non_fast_forward / pull_request / required_status_checks の4件を返すことを確認した。必須チェックは `audit`（integration_id 15368）1件のみ。
- **訂正（自分の誤り）**: 直前の commit `79cb6c9` の message と本項で「新しい `verify-and-merge.yml` が master に載る前に ruleset を有効化すると `claude/**` の反映が止まる」と書いたが、**これは誤りである。**`on: push` の workflow は push されたブランチ自身のコピーで実行されるため、master に何が載っていても既存ブランチの挙動は変わらない。順序の制約は存在しなかった。実測: `master` と `claude/figma-to-code-rules-check-exdsil` はどちらも旧workflow（直接push）を持ち、`codex/figma-to-code` は `verify-and-merge.yml` 自体を持たない。したがって残る影響は「旧workflowを持つブランチのmerge stepが拒否される」ことだけで、これはmasterの内容とは無関係であり、未検査マージが起きるわけでもない。当該ブランチが新workflowを取り込めば解消する。
- 検証の限界: `git push --dry-run origin HEAD:master` は exit 0 を返したが、**--dry-run は ref を更新せずサーバ側の ruleset 判定を通らないため、保護の証明にならない。**根拠は `rules/branches/master` の応答のみとする。実push での確認は、未検査commitをmasterへ入れる危険があるため行っていない。
...
- 事実（実測）: 共有部品の排他所有台帳が、パスをエージェント名へ常設で割り当てており、`grantedForScope` に相当する情報を持たないため scope が close しても所有が解放されない。所有者側に `active` / `waiting` のscopeが0件でも別担当は対象を宣言できず、close受領証を作れず、pre-commit が commit を拒否した。今回は6日前に close した scope 由来の所有が、無関係な実装19ファイルをせき止めていた。同じ形は 2026-08-26 にも発生しており（waiting のまま2日間握られた共有enqueueを避けるため、テンプレート内 `wp_enqueue_style()` の回避実装が入った）、そのときは台帳を手で解除しただけで機構を直していない。所有者未登録のパスも同じ理由で停止するため、過去には停止回避としてディレクトリ全体を1担当へ与えるglobが足され、それが今回の停止の原因になった。
- 変更（`templates/verify/scope-conflict-audit.mjs`）: (1) 台帳の行に `grantedForScope` を持たせ、その scope が `closed` / `aborted` / 台帳に無い状態なら所有を失効させ、以後の解決で読み飛ばす。持たない行は恒久所有として互換を保ち、件数を notes で報告する。(2) 所有者未登録を停止事由から外し、note へ降格した。並行scope同士の排他は受領証のclaim交差判定が担っており、所有台帳はその上に重なる二枚目の関門である。(3) 所有を理由に止めるときは、該当行・所有者が休眠かどうか・そのまま貼れる台帳差分を出力する。
- 規則: `rules/figma-scope-lock.md` に「共有パスの排他所有はscopeに束ねる」を追加（154行 / 上限600行、必読合計 129,916 bytes / 上限 143,360 bytes、`rule-size-audit` PASS）。
- 負のE2E: `templates/verify/scope-conflict-audit.e2e.mjs` を新設（6件）。失効の読み飛ばし、稼働中の所有による停止、休眠所有の明示と解除差分、未登録を止めないこと、交差する並行scopeが止まること、不正な `grantedForScope` の拒否を固定する。**修正前の実装に対して実際に落ちることを確認済み**（8チェックがFAIL。止める側の2件は旧実装でも通るため差分に現れない）。
- 配布: `verifier-distribute.mjs` で案件へ配布し、案件側E2Eが全て通ることを確認した（figma-gate 513 assertions / figma-page-coverage 22 / scope-conflict-audit 6）。
- 緩和の申告: 「所有者未登録は停止」を撤廃したのは**保護の弱体化**である。撤廃してよいと判断した根拠は、同時実行の防止という目的がclaim交差判定だけで達成されていること、close受領証が全変更ファイルの被覆を引き続き要求すること、そして停止を残すと未登録回避のglobが再び足されることの3点である。独立レビューはこの判断を最初の対象とすること。
- ⚠️ 未了: **独立レビューが付いていない。**実装役は Claude であり、レビュー役は actor か contextId が異なる必要がある。owner が担当を指定するまで、本件は昇格済みではなく「実装済み・レビュー待ち」として扱う。
