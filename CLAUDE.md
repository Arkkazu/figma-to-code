...
**案件で作業する場合**、エージェントのcwdになりうるディレクトリ（リポジトリのルート、テーマディレクトリなど）に入口2枚が設置されているかを `node C:\AI\figma-to-code\tools\project-entry-install.mjs <ディレクトリ> [<ディレクトリ> ...] --check` で確認します。未設置・世代差のままFigma実装を開始しないでください。
